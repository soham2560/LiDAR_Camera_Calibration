#include <calc_tools/preprocess/preprocess.hpp>
...
#include <calc_tools/preprocess/generate_lidar_image.hpp>
#include <calc_tools/preprocess/static_point_cloud_integrator.hpp>
#include <calc_tools/preprocess/pcl_densifier.hpp>
...
bool Preprocess::run(int argc, char** argv) {
  ...
  if (bag_filenames.empty()) {
    std::cerr << calc_tools::console::bold_red << "error: no input bags!!" << calc_tools::console::reset << std::endl;
    return 1;
  }
  ...
  if (vm.count("bag_id")) {
    ...
    std::cerr << calc_tools::console::bold_yellow << "use only " << bag_filenames[bag_id] << calc_tools::console::reset << std::endl;
  ...
  if (vm.count("first_n_bags")) {
    ...
    std::cerr << calc_tools::console::bold_yellow << "use only the following rosbags:" << calc_tools::console::reset << std::endl;
    for (const auto& bag_filename : bag_filenames) {
      std::cerr << calc_tools::console::bold_yellow << "- " << bag_filename << calc_tools::console::reset << std::endl;
    }
  }
  ...
  const double lidar_fov = calc_tools::estimate_lidar_fov(lidar_points.front());
    ...
    for (const auto& [topic, type] : topics_and_types) {
      ...
      if (type.find("CameraInfo") != std::string::npos) {
        if (!camera_info_topic.empty()) {
          std::cerr << calc_tools::console::bold_yellow << "warning: bag constains multiple camera_info topics!!" << calc_tools::console::reset << std::endl;
        }
        camera_info_topic = topic;
      } else if (type.find("Image") != std::string::npos) {
        if (!image_topic.empty()) {
          std::cerr << calc_tools::console::bold_yellow << "warning: bag constains multiple image topics!!" << calc_tools::console::reset << std::endl;
        }
        image_topic = topic;
      } else if (type.find("PointCloud2") != std::string::npos) {
        if (!points_topic.empty()) {
          std::cerr << calc_tools::console::bold_yellow << "warning: bag constains multiple points topics!!" << calc_tools::console::reset << std::endl;
        }
        points_topic = topic;
      }
    }
...
std::string Preprocess::get_intensity_channel(const boost::program_options::variables_map& vm, const std::string& bag_filename, const std::string& points_topic) {
  ...
  if (intensity_channel == "auto") {
    std::cerr << calc_tools::console::bold_red << "error: failed to determine point intensity channel automatically" << calc_tools::console::reset << std::endl;
    std::cerr << calc_tools::console::bold_red << "     : you must specify the intensity channel to be used manually" << calc_tools::console::reset << std::endl;
  }
...
std::tuple<std::string, cv::Size, std::vector<double>, std::vector<double>> Preprocess::get_camera_params(
  ...
  if (image_size.width == 0 && image_size.height == 0) {
    std::cerr << calc_tools::console::bold_yellow << "warning: image size is not set (image_topic=" << image_topic << ")" << calc_tools::console::reset << std::endl;
  }
  ...
  if (camera_model != "auto") {
    ...
    if (!valid_camera_models.count(camera_model)) {
      std::cerr << calc_tools::console::bold_red << "error: invalid camera model " << camera_model << calc_tools::console::reset << std::endl;

      std::stringstream sst;
      for (const auto& model : valid_camera_models) {
        sst << " " << model;
      }
      std::cerr << calc_tools::console::bold_red << "     : supported camera models are" << sst.str() << calc_tools::console::reset << std::endl;
    ...
    if (camera_model == "equirectangular") {
      ...
      if (!vm.count("camera_intrinsics")) {
        std::cerr << calc_tools::console::bold_red << "error: camera_intrinsics has not been set!!" << calc_tools::console::reset << std::endl;
      }
      if (!vm.count("camera_distortion_coeffs")) {
        std::cerr << calc_tools::console::bold_red << "error: camera_distortion_coeffs has not been set!!" << calc_tools::console::reset << std::endl;
      }
...
std::pair<cv::Mat, Frame::ConstPtr> Preprocess::get_image_and_points(
  ...
  if (!image.data) {
    std::cerr << calc_tools::console::bold_red << "error: failed to obtain an image (image_topic=" << image_topic << ")" << calc_tools::console::reset << std::endl;
    abort();
  }
  ...
  std::unique_ptr<calc_tools::PointCloudIntegrator> points_integrator;

  if (vm.count("dynamic_lidar_integration")) {
    calc_tools::DynamicPointCloudIntegratorParams params;
    ...
    points_integrator.reset(new calc_tools::DynamicPointCloudIntegrator(params));
  } else {
    calc_tools::StaticPointCloudIntegratorParams params;
    params.visualize = vm.count("visualize");
    params.voxel_resolution = vm["voxel_resolution"].as<double>();
    params.min_distance = vm["min_distance"].as<double>();
    points_integrator.reset(new calc_tools::StaticPointCloudIntegrator(params));
  }
  ...
  while (true) {
    ...
    if (!time_keeper.process(raw_points)) {
      std::cerr << calc_tools::console::yellow << "warning: skip frame with an invalid timestamp!!" << calc_tools::console::reset << std::endl;
      continue;
    }
